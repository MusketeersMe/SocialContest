...
apache::vhost { 'socialcontest.dev':
  server_name   => 'socialcontest.dev',
  serveraliases => [
    'www.socialcontest.dev'
  ],
  docroot       => '/var/www/web',
  port          => '80',
  env_variables => [
...
exec { "vendorupdate" :
    command => "/usr/local/bin/composer update --prefer-dist -d /var/www/",
    environment => "COMPOSER_HOME=/usr/local/bin/composer",
    require => [ Package["php5"], Package["git-core"], Class["composer"] ],
    timeout => 0,
    tries   => 10,
}
